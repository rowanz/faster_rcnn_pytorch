...
def _eval_detections_img(pred_classes, gt_classes, pred_boxes, gt_boxes):
    """
    For a given image, evaluate it
    :param pred_classes: 
    :param gt_classes: 
    :param pred_boxes: 
    :param gt_boxes: 
    :return: The IOUs for each of the GT classes.
    """
    for gt, gt_box in zip(gt_classes, gt_boxes):
        keep = np.array([pred == gt for pred in pred_classes], dtype=bool)
        if not np.any(keep):
            continue
        boxes = pred_boxes[keep,:]
        sub_iou = iou(gt_box[:4], boxes[:,:4])
...
def eval_detections(pred, target, mode='sg_det', iou_threshold=0.5):
    """
    Function that evaluates visual genome detections
    :param pred: Predicted detections, as an ROIDB entry
    :param target: 
    :param mode: Can be either 'sg_cls' for classification of the ground truth bounding boxes, or
                               'sg_det' for classification of custom bounding boxes
    :param iou_threshold: 
    :return: 
    """
    # gt
    gt_inds = np.where(target['max_overlaps'] == 1)[0]
    gt_boxes = target['boxes'][gt_inds].copy().astype(float)
    num_gt_boxes = gt_boxes.shape[0]
    gt_classes = target['gt_classes'].copy()
    gt_class_scores = np.ones(num_gt_boxes)

    # pred
    box_preds = pred['boxes']
    num_boxes = box_preds.shape[0]
    class_preds = pred['scores']
